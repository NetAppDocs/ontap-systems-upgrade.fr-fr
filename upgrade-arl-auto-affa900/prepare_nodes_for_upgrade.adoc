...
** `network interface show -curr-node _nodename_ -role cluster,intercluster,node-mgmt,cluster-mgmt,data`
...
NOTE: Si NetApp Volume Encryption utilisant Onboard Key Manager est en cours d'utilisation, conservez la phrase secrète du gestionnaire de clés pour terminer la resynchronisation du gestionnaire de clés plus tard dans la procédure.
