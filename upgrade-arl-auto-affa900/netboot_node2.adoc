...
[role="lead"]
Après avoir remplacé les modules système node2 correspondants, vous devrez peut-être les initialiser en réseau. Le terme netboot signifie que vous démarrez à partir d'une image ONTAP stockée sur un serveur distant. Lorsque vous vous préparez à netboot, vous devez placer une copie de l'image de démarrage ONTAP 9 sur un serveur web auquel le système peut accéder.
