...
[role="lead"]
Vous devez vérifier que les ports physiques du nœud 2 correspondent correctement aux ports physiques du nœud 4. Cela permet au nœud 4 de communiquer avec les autres nœuds du cluster et avec le réseau après la mise à niveau.
...
Si les ports physiques du nœud 2 ne correspondent pas directement aux ports physiques du nœud 4, la section suivante <<restore_network_configuration_on_node4_9151,Restaurez la configuration du réseau sur le noeud 4>> doit être utilisée pour rétablir la connectivité réseau.
...
À ce stade de la procédure, l'opération s'interrompt lorsque le nœud4 rejoint le quorum.



== Étape 1 : Vérifiez que le nœud 4 est en quorum

Vérifiez que le nœud 4 est en quorum. Reprenez ensuite l'opération de remplacement du contrôleur.
...
== Étape 2 : Restaurer la configuration réseau sur le nœud 4
...
.Et la suite ?
Après avoir vérifié l'installation de node4, link:restore_key-manager_config_node4.html["restaurer la configuration du gestionnaire de clés sur le nœud4"].
