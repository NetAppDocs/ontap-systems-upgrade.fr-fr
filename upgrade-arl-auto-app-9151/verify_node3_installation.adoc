...
[role="lead"]
Vous devez vérifier que les ports physiques du nœud1 correspondent correctement aux ports physiques du nœud3. Cela permet au nœud3 de communiquer avec les autres nœuds du cluster et avec le réseau après la mise à niveau.
...
La configuration physique des ports peut varier selon le modèle des nœuds. Au démarrage du nouveau nœud, ONTAP tente de déterminer quels ports doivent héberger les LIF du cluster afin d'atteindre automatiquement le quorum.

Si les ports physiques du nœud 1 ne correspondent pas directement aux ports physiques du nœud 3, la section suivante <<restore_network_configuration_on_node3_9151,Restaurez la configuration du réseau sur le noeud 3>> doit être utilisée pour rétablir la connectivité réseau.
...
À ce stade de la procédure, l'opération s'interrompt lorsque le nœud3 rejoint le quorum.



== Étape 1 : Vérifier que node3 est en quorum

Vérifiez que le nœud3 est en quorum. Reprenez ensuite l'opération de remplacement du contrôleur.
...
. Le remplacement du contrôleur est interrompu pour intervention avec le message suivant :
+
....
...
== Étape 2 : Restaurer la configuration réseau sur le nœud 3
...
+
Lors de la réparation de l'accessibilité, ONTAP tente d'affecter les ports aux domaines de diffusion appropriés. Toutefois, si ONTAP ne parvient pas à déterminer l'accessibilité d'un port et que celui-ci n'appartient à aucun des domaines de diffusion existants, ONTAP crée de nouveaux domaines de diffusion pour ces ports.
...
.Et la suite ?
Après avoir vérifié l'installation de node3, link:restore_key-manager_configuration_node3.html["restaurer la configuration du gestionnaire de clés sur le nœud 3"].
