...
+
Effectuez les connexions suivantes en suivant les instructions des _Instructions d'installation et de configuration_ pour la plate-forme node4, le document d'étagère de disque approprié et la _gestion des paires HA_.
...
. [[auto_check_4_step11]]Lorsque le nœud 4 s'arrête au menu de démarrage, réaffectez les disques du nœud 2 au nœud 4 en exécutant la commande suivante sur le nœud 4 :
+
...
NOTE: Dans l'exemple de sortie console ci-dessus, ONTAP vous invite à saisir le nom du nœud partenaire si le système utilise des disques à partitionnement de disque avancé (ADP).

. [[auto_check_4_step27]]À l'invite LOADER, démarrez le nœud :
+
`boot_ontap menu`
...
.Et la suite ?
Après l'installation et le démarrage de node4, link:verify_node4_installation.html["vérifier l'installation de node4"].
