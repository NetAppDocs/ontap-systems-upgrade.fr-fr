...
[role="lead"]
Il existe plusieurs méthodes de relocalisation agrégée (ARL) pour la mise à niveau du matériel des contrôleurs. Cette procédure ARL décrit comment mettre à niveau une paire haute disponibilité de contrôleurs dans les systèmes de stockage AFF et FAS introduits dans ONTAP 9.15.1 ou version ultérieure avec de nouveaux contrôleurs, tout en conservant les disques existants et les données stockées sur les disques.
...
Lors d'une mise à niveau de contrôleur, le contrôleur existant est remplacé par un autre contrôleur exécutant une plateforme plus récente ou plus performante. Ce contenu fournit les étapes de la procédure partiellement automatisée, qui utilise des vérifications automatiques de la connectivité des ports réseau afin de simplifier davantage l'expérience de mise à niveau du contrôleur.

.Et la suite ?
Après avoir pris connaissance de cette procédure de mise à niveau ARL link:decide_to_use_the_aggregate_relocation_guide.html["décider s'il convient d'utiliser cette procédure"].
