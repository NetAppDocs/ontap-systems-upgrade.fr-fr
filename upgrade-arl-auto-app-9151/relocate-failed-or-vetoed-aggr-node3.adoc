...
summary: 'Déplacez manuellement les agrégats défaillants ou refusés vers le nœud 3 lors de la mise à niveau des contrôleurs exécutant ONTAP 9.15.1 ou version ultérieure à l"aide de `system controller replace` commandes.' 
---
= Déplacer les agrégats défaillants ou refusés vers le nœud 3
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
.Et la suite ?
Après avoir déplacé les agrégats ayant échoué ou ayant été rejetés vers le nœud 3, link:retire_node2.html["retirer node2"].
