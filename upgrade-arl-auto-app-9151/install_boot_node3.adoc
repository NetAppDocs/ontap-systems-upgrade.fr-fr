...
. Lorsque le nœud 3 s'arrête au menu de démarrage, réaffectez les disques du nœud 1 au nœud 3 en exécutant la commande suivante sur le nœud 3 :
+
...
. Saisissez  `Ctrl-C` pendant le démarrage automatique pour arrêter le nœud à l' `LOADER>`invite.
...
.Et la suite ?
Après avoir installé et démarré node3, link:verify_node3_installation.html["vérifiez l'installation de node3"].
