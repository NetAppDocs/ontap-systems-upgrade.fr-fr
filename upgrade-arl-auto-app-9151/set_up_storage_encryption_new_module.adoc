...
summary: 'Si nécessaire, configurez le cryptage du stockage, installez les certificats SSL et configurez les serveurs de gestion des clés lors de la mise à niveau des contrôleurs exécutant ONTAP 9.15.1 ou une version ultérieure à l"aide de `system controller replace` commandes.' 
---
...
.Et la suite ?
Après avoir configuré le chiffrement du stockage sur le nouveau module de contrôleur, link:set_up_netapp_volume_encryption_new_module.html["Configurez le chiffrement de volume ou d’agrégat NetApp sur le nouveau module contrôleur"].
