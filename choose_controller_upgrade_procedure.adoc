...
| Est-ce que j'utilise une version logicielle différente sur les nœuds d'origine et les nouveaux nœuds ? | Mettez à niveau la version d'ONTAP sur les nœuds d'origine vers la version la plus récente prise en charge. Dans la mesure du possible, les nouveaux nœuds doivent prendre en charge la version d'ONTAP exécutée sur les nœuds d'origine. Si cela n'est pas possible, link:https://docs.netapp.com/us-en/ontap/upgrade/prepare.html["Mettez à niveau la version ONTAP"^] sur les nœuds d'origine vers la version la plus récente prise en charge. Vous ne pouvez pas avoir une différence de version supérieure à quatre entre les nœuds d'origine et les nouveaux nœuds. Par exemple, ONTAP 9.8 et 9.12.1 sont pris en charge ; cependant, ONTAP 9.8 et 9.13.1 ne sont pas pris en charge. link:https://docs.netapp.com/us-en/ontap/upgrade/concept_mixed_version_requirements.html["En savoir plus sur les clusters ONTAP à versions mixtes"^]. 


| Est-ce que je souhaite effectuer une mise à niveau radicale ? | link:upgrade/upgrade-decide-to-use-this-guide.html["Mettre à niveau en déplaçant le stockage"]. Cela nécessite un nouveau système doté de son propre espace de stockage et de ses propres étagères pour conserver les données du système d'origine. Il est possible de convertir une étagère de contrôleur pour certaines mises à niveau. 
...
* Convertissez le système d'origine en baie de disques et déplacez le stockage :
+
** link:upgrade/upgrade_aff_a250_to_aff_a400_ndu_upgrade_workflow.html["Convertissez AFF A250 en shelf de disques et déplacez le stockage vers AFF A400"]
** link:upgrade/convert-to-drive-shelf.html["Convertissez AFF A150, AFF A220 ou FAS2820 en baie de disques et transférez le stockage vers un système de remplacement compatible"]
...
| Ai-je un système modulaire ? *Remarque* : Dans un système modulaire, le contrôleur ne possède pas de disques internes ; tout le stockage se trouve dans des étagères externes. | link:upgrade-arl/index.html["Mise à niveau via le transfert d'agrégats"]. 
...
| Ai-je un système intégré et souhaite-je le mettre à niveau en remplaçant uniquement le module contrôleur, en conservant le châssis et les disques existants ? | link:upgrade-arl-auto-in-chassis/index.html["Mise à niveau via ARL, en conservant le même châssis, les mêmes disques et les données stockées sur les disques"]. 
...
Cet exemple fournit des réponses possibles pour une mise à niveau du contrôleur FAS9500 vers FAS90.
