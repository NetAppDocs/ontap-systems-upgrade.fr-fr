...
summary: 'Si nécessaire, configurez le cryptage du stockage, installez les certificats SSL et configurez les serveurs de gestion des clés lors d"une mise à niveau manuelle du transfert d"agrégats sur les contrôleurs exécutant ONTAP 9.8 ou une version ultérieure.' 
---
...
.Et la suite ?
Après avoir configuré le chiffrement du stockage sur le nouveau module de contrôleur, link:set_up_netapp_encryption_on_new_controller.html["Configurez le chiffrement de volume ou d’agrégat NetApp sur le nouveau module contrôleur"].
