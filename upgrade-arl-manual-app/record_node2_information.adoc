...
NOTE: Il est possible que votre système ne dispose pas d'intercluster LIFs. Vous ne possédez une cluster management LIF que sur un seul nœud d'une paire de nœuds. Une cluster management LIF est affichée dans l'exemple de sortie de link:record_node1_information.html#step1["Étape 1"] dans _Record node1 port information_.
...
+
Les domaines de diffusion doivent être associés aux ports du nouveau contrôleur ultérieurement dans la procédure.
...
.Et la suite ?
Après l'enregistrement des informations du nœud 2, link:retire_node2.html["retirer node2"].
