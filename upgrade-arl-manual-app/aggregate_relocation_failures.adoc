...
+
Pour des informations détaillées sur les options  `storage aggregate relocation start`,  `override-vetoes` et  `override-destination-checks`, reportez-vous à link:other_references.html["Références"] pour accéder à la _ONTAP 9 Command reference_.
...
* Lors de l'étape 3, les agrégats sont déplacés du nœud2 vers le nœud3. Certains des agrégats déplacés ont le nœud1 comme port d'attache. Par exemple, un tel agrégat pourrait être appelé aggr_node_1. Si le déplacement de aggr_node_1 échoue lors de l'étape 3 et que le déplacement ne peut pas être forcé, alors l'agrégat reste sur le nœud2.
* Après l'étape 4, lorsque le nœud2 est remplacé par le nœud4. Lorsque le nœud2 est remplacé, aggr_node_1 passe en ligne avec le nœud4 comme port d'attache au lieu du nœud3.
