...
+
Accédez à link:other_references.html["Références"] pour accéder au _System Administration Reference_ pour des informations sur les SP et au _ONTAP 9 Command reference_ pour des informations détaillées sur la commande  `system service- processor network modify`.
...
.Et la suite ?
Après avoir confirmé que les nouveaux contrôleurs sont correctement configurés, link:set_up_storage_encryption_new_controller.html["Configurez le chiffrement du stockage sur le nouveau module contrôleur"].
