...
.Étapes
. [[relocate-step1]]Relocalisez les agrégats non racines en complétant les sous-étapes suivantes :
+
...
+
Les résultats s'affichent  `Done` pour un agrégat après son déplacement.
...
Consultez link:other_references.html["Références"] pour accéder au contenu _Disk and aggregate management with the CLI_ et au _ONTAP 9 Command reference_ pour plus d'informations sur les commandes de relocalisation des agrégats de stockage.
...
.Et la suite ?
Après avoir déplacé les agrégats non racines du nœud1 vers le nœud2, link:move_nas_lifs_node1_node2.html["déplacer les LIF de données NAS appartenant au node1 vers le node2"].
