...
. Capturer les informations dans la sortie de la commande dans ,Étape 3 à utiliser dans la section link:map_ports_node1_node3.html["Mapper les ports du node1 vers le node3"].
+
...
| N'a pas enregistré le numéro d'ID système NVRAM dans la section link:prepare_nodes_for_upgrade.html["Préparer les nœuds pour la mise à niveau"] | Terminé ,Étape 11 et ,Étape 12 puis continuez à link:retire_node1.html["Retirer le nœud 1"]. 
|===
