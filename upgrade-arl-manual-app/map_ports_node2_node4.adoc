...
[role="lead"]
Vous devez vous assurer que les ports physiques du nœud2 correspondent correctement aux ports physiques du nœud4. Cela permet au nœud4 de communiquer avec les autres nœuds du cluster et avec le réseau après la mise à niveau.
...
.. Lors de la réparation de l'accessibilité, ONTAP tente d'affecter les ports aux domaines de diffusion appropriés. Toutefois, si ONTAP ne parvient pas à déterminer l'accessibilité d'un port et que celui-ci ne correspond à aucun des domaines de diffusion existants, ONTAP tente de créer de nouveaux domaines de diffusion pour ces ports. Le cas échéant, vous pouvez supprimer les nouveaux domaines de diffusion créés si tous leurs ports membres deviennent des ports membres des groupes d'interfaces. Supprimer des domaines de diffusion :
+
...
. [[man_map_2_Step5]]À partir d'ONTAP 9.8, ONTAP modifie automatiquement les ports d'attache des LIF si ces ports sont déplacés entre les domaines de diffusion lors de la procédure de réparation de l'accessibilité des ports réseau. Si le port d'attache d'une LIF a été déplacé vers un autre nœud ou n'est pas attribué, cette LIF est présentée comme une LIF déplacée. Restaurez les ports d'attache des LIF déplacées dont les ports d'attache n'existent plus ou ont été déplacés vers un autre nœud.
+
...
.Et la suite ?
Après avoir mappé les ports du nœud 2 au nœud 4, link:move_nas_lifs_node2_from_node3_node4_verify_san_lifs_node4.html["Déplacer les LIF de données NAS appartenant au nœud2 du nœud3 vers le nœud4 et vérifier les LIF SAN sur le nœud4"].
