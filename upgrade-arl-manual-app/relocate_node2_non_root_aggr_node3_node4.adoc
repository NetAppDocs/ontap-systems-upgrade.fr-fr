...
Pour plus d'informations sur les commandes de relocalisation des agrégats de stockage, reportez-vous à link:other_references.html["Références"] pour accéder à _Disk and aggregate management with the CLI_ et au _ONTAP 9 Command reference_.
...
.Et la suite ?
Après avoir déplacé les agrégats non-root du nœud2 du nœud3 vers le nœud4, link:manage_authentication_kmip.html["gérer l'authentification à l'aide des serveurs KMIP"].
