...
. Si vous avez installé des lecteurs NetApp Storage Encryption (NSE), procédez comme suit :
+
...
+
Saisissez la phrase secrète et les informations de sauvegarde que vous avez enregistrées précédemment dans la procédure. Voir link:manage_authentication_okm.html["Gérez les clés d'authentification à l'aide du gestionnaire de clés intégré"].
...
| Ne dispose pas de la version ONTAP correcte ou actuelle du système node4 | Aller à<<man_install4_Step13,Étape 13>> . 


| Dispose de la version correcte ou à jour de ONTAP sur le nœud 4 | Aller à<<man_install4_Step18,Étape 18>> . 
|===
. [[man_install4_Step13]]Configurez la connexion netboot en choisissant l’une des actions suivantes.
+
...
. [[man_install4_Step18]]Sélectionnez le mode de maintenance `5` depuis le menu de démarrage et entrez `y` lorsque vous êtes invité à continuer le démarrage.
. [[man_install4_Step19]]Avant de continuer, allez àlink:set_fc_uta_uta2_config_node4.html["Définissez la configuration FC ou UTA/UTA2 sur le nœud 4"] pour apporter les modifications nécessaires aux ports FC ou UTA/UTA2 sur le nœud.  Effectuez les modifications recommandées dans ces sections, redémarrez le nœud et passez en mode Maintenance.
...
+
La  `disk reassign`commande ne réaffecte que les disques pour lesquels  `_node2_sysid_` est le propriétaire actuel.
...
. Vérifiez que l'ID du système partenaire est correctement défini comme indiqué dans<<man_install4_Step19,Étape 19>> sous option.
+
...
.Et la suite ?
Après l'installation et le démarrage de node4, link:set_fc_uta_uta2_config_node4.html["définir la configuration FC ou UTA/UTA2 sur node4"].
