...
Certains ou tous les agrégats ont été déplacés de node1 vers node2, et la haute disponibilité (HA) reste activée. Node2 prend en charge le volume racine de node1 ainsi que tous les agrégats non racines qui n'ont pas été déplacés.

.Description de la tâche
La propriété des agrégats déplacés est identique à celle des agrégats non racines repris, car le propriétaire initial n'a pas changé. Lorsque le nœud 1 entre  `waiting for giveback state`en jeu, le nœud 2 lui restitue tous les agrégats non racines du nœud 1.
...
.Description de la tâche
Le nœud 1 récupère tous les agrégats du nœud 2, ainsi que tous ses propres agrégats qu'il avait déplacés vers le nœud 2. Lorsque le nœud 2 entre dans l' `Waiting for Giveback`état, le nœud 1 lui restitue tous les agrégats du nœud 2.
...
. Afficher le nœud 2.
+
Une interruption de service client se produit pour tous les agrégats pendant le démarrage du nœud 2.
...
Le nœud 3 ne prend pas le relais suite à une panne du nœud 2 car la haute disponibilité est déjà désactivée.
...
Le nœud 2 a déplacé tout ou partie de ses agrégats vers le nœud 3, qui fournit les données à partir des agrégats qui ont été déplacés. HA est désactivé.

.Description de la tâche
Une interruption de service est constatée chez les clients dont les agrégats n'ont pas été déplacés.
...
Si le nœud 3 tombe en panne pendant que le nœud 2 déplace des agrégats vers le nœud 3, le nœud 2 interrompt le déplacement de tous les agrégats restants.
...
. Afficher le nœud 3.
+
Une panne client affecte tous les agrégats.
