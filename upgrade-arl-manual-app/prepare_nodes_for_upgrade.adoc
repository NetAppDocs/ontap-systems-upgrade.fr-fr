...
WARNING: N'essayez pas d'effacer le contenu de la NVRAM .  S'il est nécessaire d'effacer le contenu de la NVRAM, contactez le support technique NetApp .
...
+
Si ni le nœud 1 ni le nœud 2 ne possèdent des agrégats dont ils sont le propriétaire actuel (mais pas le port d'attache), le système renvoie un message similaire à l'exemple suivant :
...
+
Pour des informations détaillées sur la commande  `network interface revert`, consultez link:other_references.html["Références"] pour accéder à la _ONTAP 9 Command reference_.
...
+
Les VLAN sont configurés sur des ports physiques. Si les ports physiques changent, alors les VLAN doivent être recréés plus tard dans la procédure.
...
+
Si ni iSCSI ni FC ne sont configurés pour la SVM, la commande affiche un message similaire à l'exemple suivant :
...
+
Si iSCSI ou FC est configuré pour la SVM, la commande affiche un message similaire à l'exemple suivant :
...
+
Vous pouvez réutiliser les paramètres réseau des dispositifs de gestion à distance, dans ce cas les SP, du système d'origine pour les SP des nouveaux nœuds. Pour des informations détaillées sur le SP, consultez link:other_references.html["Références"] pour accéder au _System Administration Reference_ et au _ONTAP 9 Command reference_.
...
+
Pour des informations détaillées sur AutoSupport, reportez-vous à link:other_references.html["Références"] pour accéder au _System Administration Reference_ et au _ONTAP 9 Command reference_.
...
.Et la suite ?
Après avoir préparé les nœuds pour la mise à niveau, link:manage_authentication_okm.html["Gérez les clés d'authentification à l'aide de l'Onboard Key Manager"].
