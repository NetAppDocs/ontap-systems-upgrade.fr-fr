...
Pour plus d'informations sur les commandes de relocalisation des agrégats de stockage, allez sur link:other_references.html["Références"] pour accéder à _Disk and aggregate management with the CLI_ et au _ONTAP 9 Command reference_.
...
.Et la suite ?
Après avoir déplacé les agrégats non racines du nœud 2 vers le nœud 3, link:move_nas_lifs_node2_node3.html["déplacer les LIF de données NAS appartenant à node2 vers node3"].
