...
summary: 'Migrez les interfaces logiques (LIF) et les agrégats de données sur le nœud2 vers le nœud1 avant de convertir le nœud2 en un châssis de disques.' 
---
...
[role="lead"]
Migrez les interfaces logiques (LIF) et les agrégats de données sur AFF A150, AFF A220 ou FAS2820 node2 vers node1 avant de convertir node2 en un châssis de disques.

.Avant de commencer
Vérifiez que vous répondez aux exigences suivantes :

* Les contrôleurs d'origine et de remplacement ont la même version d'ONTAP et le même correctif, dans la mesure du possible. Consultez la  https://hwu.netapp.com["NetApp Hardware Universe"^] pour les versions d'ONTAP prises en charge.
...
.Étapes
Effectuez ces étapes sur le nœud1.
...
.Et la suite ?
link:convert-node2-to-drive-shelf.html["Convertissez le nœud 2 en tiroir disque et connectez-vous au nœud 4"]
