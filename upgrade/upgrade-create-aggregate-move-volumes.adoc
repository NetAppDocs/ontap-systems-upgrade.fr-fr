...
+
Le nouvel agrégat et l'ancien agrégat à partir duquel le volume est déplacé doivent appartenir au même SVM.
...
.Et la suite ?
link:upgrade-move-lifs-to-new-nodes.html["Déplacez les LIF de données non-SAN et les LIF de cluster-management vers les nouveaux nœuds"]
