...
[role="lead"]
Vous devez vous assurer que les ports physiques du nœud1 correspondent correctement aux ports physiques du nœud3. Cela permet à nœud3 de communiquer avec les autres nœuds du cluster et avec le réseau après la mise à niveau.
...
Vous devez rendre la configuration des ports et des LIF du nœud d'origine compatible avec celle que vous prévoyez pour le nouveau nœud. En effet, le nouveau nœud rejoue la même configuration au démarrage, ce qui signifie qu'au démarrage de node3, ONTAP tente d'héberger les LIF sur les mêmes ports que ceux utilisés sur node1.

Par conséquent, si les ports physiques du nœud1 ne correspondent pas directement aux ports physiques du nœud3, des modifications de la configuration logicielle sont nécessaires pour rétablir la connectivité du cluster, de la gestion et du réseau après le démarrage. De plus, si les ports du cluster du nœud1 ne correspondent pas directement aux ports du cluster du nœud3, le nœud3 pourrait ne pas rejoindre automatiquement le quorum lors de son redémarrage tant qu'une modification de la configuration logicielle n'aura pas été effectuée pour héberger les LIF du cluster sur les ports physiques corrects.
...
|===
+
Reportez-vous à link:record_node1_information.html["Enregistrer les informations sur le noeud 1"] pour connaître les étapes permettant d’obtenir ces informations.
...
----
+
Accédez à link:other_references.html["Références"] pour accéder à _Network Management_ ou au _ONTAP 9 Command reference_ pour plus d'informations.
