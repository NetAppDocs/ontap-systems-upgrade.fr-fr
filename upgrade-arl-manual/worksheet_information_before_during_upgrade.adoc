...
summary: 'Collecter certaines informations lors d"une mise à niveau manuelle du transfert d"agrégats sur des contrôleurs exécutant ONTAP 9.7 ou une version antérieure.' 
---
...
[role="lead"]
Vous devez collecter certaines informations pour mettre à niveau les nœuds d'origine. Ces informations incluent les ID de nœud, les détails de port et de LIF, les clés de licence et les adresses IP.

Vous pouvez utiliser la fiche suivante pour enregistrer des informations à utiliser ultérieurement dans la procédure :
