...
* Si vous mettez à niveau un système de la série V ou un système avec le logiciel de virtualisation FlexArray connecté à des baies de stockage, vous devez effectuer <<man_install4_Step1,Étape 1>> à travers <<man_install4_Step7,Étape 7>> , laissez cette section à <<man_install4_Step8,Étape 8>> et suivez les instructions dans link:set_fc_uta_uta2_config_node4.html["Définissez la configuration FC ou UTA/UTA2 sur le nœud 4"] Si nécessaire, saisissez les commandes en mode Maintenance. Vous devez ensuite revenir à cette section et reprendre la procédure à l'adresse <<man_install4_Step9,Étape 9>> .
