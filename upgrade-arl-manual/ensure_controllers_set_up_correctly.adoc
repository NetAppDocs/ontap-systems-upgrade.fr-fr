...
. Si vous utilisez des disques à chiffrement automatique dans la configuration, et que vous avez défini le `kmip.init.maxwait` variable à `off` (par exemple, dans link:install_boot_node3.html#step16["Étape 16"] De _Install and boot node3_), vous devez annuler la variable :
+
