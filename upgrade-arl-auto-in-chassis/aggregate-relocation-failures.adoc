...
+
Pour des informations détaillées sur les  `storage aggregate relocation start`, `override-vetoes`, et  `override-destination-checks` options, reportez-vous à link:other-references.html["Références"] pour accéder au _ONTAP 9 Command reference_.
...
+
Lorsque node2 est remplacé, aggr_node_1 se met en ligne avec node1 comme nœud d'origine au lieu de node2.
...
+
Pour identifier les agrégats qui n'ont pas été correctement déplacés, reportez-vous à la liste des agrégats avec le propriétaire du node1 que vous avez obtenue dans la section link:prepare-nodes-for-upgrade.html["Préparer les nœuds pour une mise à niveau"] et comparez-la avec le résultat de la commande ci-dessus.

. Comparez le résultat de l'étape 1 avec le résultat que vous avez capturé pour node1 dans la section link:prepare-nodes-for-upgrade.html["Préparer les nœuds pour une mise à niveau"] et notez les agrégats qui n'ont pas été correctement déplacés.
