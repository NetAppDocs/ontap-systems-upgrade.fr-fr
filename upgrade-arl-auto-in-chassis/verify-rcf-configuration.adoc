...
Si vous effectuez une mise à niveau vers une configuration de cluster sans commutateur à deux nœuds, vous pouvez ignorer cette section et passer à link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["Déplacement des agrégats non racines et des LIF de données NAS vers le nœud 2"].
...
| `Up` | Allez à link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["Déplacement des agrégats non racines et des LIF de données NAS vers le nœud 2"]. 
...
.Et la suite ?
Après avoir vérifié la configuration RCF link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["Déplacez les agrégats non racines et les LIF de données NAS vers le nœud2"].
