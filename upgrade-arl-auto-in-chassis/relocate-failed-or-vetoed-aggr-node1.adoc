...
summary: 'Déplacez manuellement les agrégats défaillants ou refusés vers le nœud 1 lors de la mise à niveau des contrôleurs exécutant ONTAP 9.15.1 ou version ultérieure à l"aide de `system controller replace` commandes.' 
---
= Déplacer les agrégats défaillants ou refusés vers le nœud 1
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Si des agrégats ne parviennent pas à se déplacer ou sont rejetés, vous devez les déplacer manuellement vers le nœud 1 ou, si nécessaire, ignorer les vetos ou les vérifications de destination.
...
.Et la suite ?
Après avoir déplacé les agrégats ayant échoué ou ayant été rejetés vers le nœud 1, link:retire-node2.html["retirer node2"].
