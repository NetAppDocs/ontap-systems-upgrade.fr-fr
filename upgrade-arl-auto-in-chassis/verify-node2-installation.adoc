...
== Étape 1 : Vérifier que le nœud 2 est dans le quorum

Vérifiez que le nœud 2 est en quorum. Reprenez ensuite l'opération de remplacement du contrôleur.
...
. Le remplacement du contrôleur étant en pause, procédez à <<restore_network_configuration_on_node2,Restaurez la configuration réseau sur le noeud 2>>.




== Étape 2 : Restaurer la configuration réseau sur le nœud 2
...
.Et la suite ?
Après avoir vérifié l'installation de node2, link:restore-key-manager-config-node2.html["restaurer la configuration du gestionnaire de clés sur le nœud2"].
