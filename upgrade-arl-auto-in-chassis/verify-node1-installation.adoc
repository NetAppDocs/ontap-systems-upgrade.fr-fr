...
== Étape 1 : Vérifier que le nœud 1 est dans le quorum

Vérifiez que node1 est en quorum. Reprenez ensuite l'opération de remplacement du contrôleur.
...
. Cette étape s'applique aux configurations de mise à niveau suivantes. Pour toutes les autres mises à niveau système, ignorez cette étape et accédez à <<verify-node1-step3,Étape 3>> :
+
** Clusters sans commutateur à deux nœuds
** Systèmes AFF A250 ou AFF C250 connectés à un commutateur mis à niveau vers un système AFF A50, AFF A30, AFF C30 ou AFF C60


+
--
Si le nœud 1 ne rejoint pas le quorum automatiquement :
...
.. Migrer les LIF du cluster node1 vers e1a et e1b :
+
...
--
. [[verify-node1-step3]]Vérifiez que node1 et node2 font partie du même cluster et que le cluster est sain :
+
`cluster show`
...
. Le remplacement du contrôleur étant en pause, procédez à <<restore_network_configuration_on_node1,Restaurez la configuration réseau sur le noeud 1>>.




== Étape 2 : Restaurer la configuration réseau sur le nœud 1
...
.Description de la tâche
Pour plus d'informations sur la création et la recréation de VLAN, de groupes d'interfaces et de domaines de diffusion, reportez-vous à link:other-references.html["Références"] pour accéder au contenu _Network Management_.
...
.Et la suite ?
Après avoir vérifié l'installation du nœud 1, link:restore-key-manager-config-upgraded-node1.html["restaurer la configuration du gestionnaire de clés sur le nœud1 mis à niveau"].
