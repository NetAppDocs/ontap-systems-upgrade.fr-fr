...
[role="lead"]
Le nœud 2 équipé des modules de remplacement est désormais prêt à démarrer. Les modules de remplacement compatibles sont listés dans le link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matrice des systèmes pris en charge"].


NOTE: Vous ne déplacez les connexions de console et de gestion que lorsque vous effectuez une mise à niveau en remplaçant les modules système.

.Étapes
. [[boot_node2_step1]]Si vous avez installé des lecteurs NetApp Storage Encryption (NSE), procédez comme suit :
+
...
+
Saisissez la phrase secrète et les informations de sauvegarde que vous avez enregistrées précédemment dans la procédure. Voir link:complete-preparation-for-upgrade.html#manage-storage-encryption-using-okm["Gérez le chiffrement du stockage à l'aide du gestionnaire de clés intégré"].
...
. Lorsque le nœud s'arrête au menu de démarrage, réaffectez les anciens disques node2 au node2 de remplacement en exécutant la commande suivante sur node2 :
+
...
.Et la suite ?
Après avoir démarré le nœud 2 avec les modules système de remplacement, link:verify-node2-installation.html["vérifier l'installation de node2"].
