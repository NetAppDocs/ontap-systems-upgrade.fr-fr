...
Démarrez la procédure de remplacement du contrôleur et effectuez les vérifications préalables. Si nécessaire, identifiez le type de disques à chiffrement automatique utilisés.
...
+
Vous obtenez un résultat similaire à l'exemple suivant. Le résultat affiche la version d'ONTAP en cours d'exécution sur votre cluster :
...
....
. Sélectionnez  `y`. Vous voyez la sortie suivante :
+
....
...
| Vérification de la version de l'image | Vérifie que les nœuds remplacés exécutent la même version d'ONTAP. Si les versions des images ONTAP sont différentes, la tâche échoue. Les nouveaux nœuds doivent avoir la même version d'ONTAP 9.x installée que celle installée sur les nœuds d'origine. Si les nouveaux nœuds ont une version différente d'ONTAP installée, vous devez effectuer un netboot des nouveaux contrôleurs après leur installation. Pour obtenir des instructions sur la mise à niveau d'ONTAP, consultez link:other-references.html["Références"] pour accéder à _Upgrade ONTAP_. 
...
| Vérification de l'état du disque | Si l'un des nœuds remplacés présente des disques manquants ou défaillants, la tâche échoue. En cas de disques manquants, consultez link:other-references.html["Références"] pour accéder à _Disk and aggregate management with the CLI_, _Logical storage management with the CLI_ et _HA pair management_ afin de configurer le stockage de la paire haute disponibilité. 
...
.Et la suite ?
Après avoir préparé les nœuds, link:complete-preparation-for-upgrade.html["achever la préparation de la mise à niveau"].
