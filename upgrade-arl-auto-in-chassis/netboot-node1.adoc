...
Il n'est pas possible de vérifier la version d'ONTAP installée sur le support de démarrage du module de contrôleur de remplacement à moins qu'il ne soit installé dans un châssis et sous tension. La version d'ONTAP sur le support de démarrage du système de remplacement doit être identique à la version d'ONTAP exécutée sur l'ancien système que vous mettez à niveau, et les images de démarrage principale et de secours sur le support de démarrage doivent correspondre. Pour vérifier la version minimale d'ONTAP prise en charge pour votre mise à niveau, consultez la link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matrice des systèmes pris en charge"].
...
* Téléchargez les fichiers système nécessaires pour votre système et la version correcte d'ONTAP depuis le _NetApp Support Site_. Reportez-vous à link:other-references.html["Références"] pour accéder au _NetApp Support Site_.
...
.Étapes
. Reportez-vous à link:other-references.html["Références"] pour accéder au _NetApp Support Site_ afin de télécharger les fichiers utilisés pour effectuer le netboot du système.
...
+
.. À l'invite suivante, exécutez la  `wipeconfig` commande. Appuyez ensuite sur Entrée :
+
[listing]
----
Please choose one of the following:
...
.Et la suite ?
Après le démarrage réseau du nœud 1, link:cable-node1-for-shared-cluster-HA-storage.html["Câbler le nœud 1 pour le cluster-HA partagé et le stockage"].
