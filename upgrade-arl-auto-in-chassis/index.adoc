...
[role="lead"]
Il existe plusieurs méthodes de relocalisation agrégée (ARL) pour la mise à niveau du matériel des contrôleurs. Cette procédure décrit comment mettre à niveau les contrôleurs de stockage d'une paire haute disponibilité en convertissant le système existant en système de remplacement, tout en conservant le châssis existant, les disques et les données stockées sur les disques.
...
NOTE: Les termes *node1* et *node2* sont utilisés uniquement à titre de référence pour désigner les noms des nœuds dans ce document. Lors de l'application de la procédure, vous devez utiliser les noms réels de vos nœuds.



== Exigences et restrictions

Vous devez prendre en compte des facteurs importants avant de commencer la mise à niveau.


NOTE: Vous devez examiner toutes les informations importantes suivantes.

* Cette procédure est complexe et suppose que vous possédez des compétences avancées en administration ONTAP. Vous devez lire et comprendre la link:guidelines-for-upgrading-controllers-with-arl.html["directives pour la mise à niveau des contrôleurs"] et link:overview-of-the-arl-upgrade.html["Séquence de mise à niveau ARL"] avant de commencer la mise à niveau.
...
* Si vous possédez un commutateur qui n'est pas pris en charge par la version ONTAP et le système de remplacement vers lequel vous effectuez la mise à niveau, reportez-vous à link:other-references.html["Références"] pour accéder à _Hardware Universe_.
...
* Si vous effectuez une mise à niveau à partir d'un système existant avec interruption incluse dans le link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matrice des systèmes pris en charge"], vous pouvez mettre à niveau le matériel du contrôleur en déplaçant le stockage ou en contactant le support technique. Reportez-vous à link:other-references.html["Références"] pour accéder à _Mise à niveau par déplacement de volumes ou de stockage_.




== Processus de mise à niveau automatisé du contrôleur
...
.Et la suite ?
Après avoir pris connaissance de cette procédure de mise à niveau ARL, link:decide-to-use-the-aggregate-relocation-guide.html["décider s'il convient d'utiliser cette procédure"].
