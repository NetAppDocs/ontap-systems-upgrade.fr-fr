...
---
= Remplacez le module de contrôleur node1 AFF A800 ou AFF C800
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
Si vous n'interrompez pas AUTOBOOT à temps et que le nœud 1 commence à démarrer, attendez l'invite et appuyez sur Ctrl+C pour accéder au menu de démarrage. Après que le nœud se soit arrêté sur le menu de démarrage, utilisez l'option  `8` pour redémarrer le nœud et interrompre AUTOBOOT pendant le redémarrage.
...
.Et la suite ?
Après avoir installé le module de contrôleur AFF A90, AFF A70 ou AFF C80, link:netboot-node1.html["netboot node1"].
