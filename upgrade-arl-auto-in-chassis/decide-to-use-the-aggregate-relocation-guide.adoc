...
summary: 'Décidez s"il convient d"utiliser la procédure ARL qui met à niveau les modèles de contrôleurs dans le même châssis, les mêmes disques et les données stockées sur les disques.' 
---
...
[role="lead"]
Il existe plusieurs méthodes de relocalisation agrégée (ARL) pour la mise à niveau du matériel des contrôleurs. Ce contenu décrit comment mettre à niveau les contrôleurs de stockage d'une paire haute disponibilité en convertissant le système existant en système de remplacement, tout en conservant le châssis du système existant, les disques et les données stockées sur les disques. Vous ne devez utiliser cette procédure complexe que si vous êtes un administrateur ONTAP expérimenté.
...
+
Pour mettre à niveau une configuration IP MetroCluster, consultez link:other-references.html["Références"] pour accéder au contenu _MetroCluster Upgrade and Expansion_.
...
Le tableau suivant présente la matrice des systèmes pris en charge pour effectuer une mise à niveau du contrôleur en convertissant le système existant en système de remplacement, en conservant le châssis, les disques et les données stockées sur ceux-ci.


CAUTION: Cette procédure s'applique strictement aux configurations de mise à niveau suivantes. N'utilisez pas cette procédure pour effectuer une mise à niveau entre d'autres combinaisons de systèmes. Pour tous les autres modèles de contrôleurs, consultez link:other-references.html["Références"] pour accéder à _Use "system controller replace" commands to upgrade controller hardware running ONTAP 9.8 or later_ et _Using aggregate relocation to manually upgrade controller hardware running ONTAP 9.8 or later_.
...
| AFF A70 ^1^ | AFF A90 | 9.18.1 et versions ultérieures 


| AFF A30 ^1^ | AFF A50 | 9.18.1 et versions ultérieures 


| AFF A20 ^1^ | AFF A30 ^3^, AFF A50 | 9.18.1 et versions ultérieures 


| AFF C30 ^1^ | AFF C60 | 9.18.1 et versions ultérieures 


| FAS70 ^1^ | FAS90 | 9.18.1 et versions ultérieures 
...
====
^3^ Il existe certaines exigences de connexion de port pour une mise à niveau d'un AFF A20 vers un AFF A30 dans une configuration de cluster sans commutateur à deux nœuds :

* L'AFF A20 prend en charge X60132A (4 ports 10/25GbE CX7) et X60131A (2 ports 10/25GbE CX6-LX) pour l'interconnexion de cluster.
* L'AFF A30 ne prend en charge que le X60130A (2-port 100GbE CX6-DX) pour interconnexion de cluster.
* Utilisez une carte 10/25GbE temporaire pour connecter temporairement l'AFF A20 à l'AFF A30. Une carte 10/25GbE à 4 ports ou 2 ports prend en charge le clustering sur l'AFF A20. Il est recommandé d'utiliser la même carte dans l'AFF A20 et l'AFF A30. Si vous ne disposez pas d'une nouvelle carte, vous pouvez utiliser la carte de l'AFF A20 comme interconnexion de cluster temporaire.
* Lorsque AFF A30 node3 est mis en ligne, il configure les ports HA IC sur 100 GbE et tente de placer les LIFs du cluster sur les ports 10/25 GbE. ONTAP détecte ces ports comme étant connectés à l'AFF A20. Si ONTAP ne détecte pas automatiquement ces ports, vous pouvez modifier manuellement les ports temporaires du cluster. Ensuite, si nécessaire, vous pouvez déplacer les LIFs du cluster vers les ports temporaires du cluster.
* Après la mise à niveau de AFF A20 node1 et la mise hors service de AFF A20 node2 en préparation de la mise à niveau, vous devez retirer les connexions des ports temporaires de cluster 10/25GbE et utiliser uniquement les ports partagés de cluster 100GbE. Lorsque AFF A30 node3 et node4 sont connectés via les ports 100GbE, les connexions sont établies et vous pouvez terminer la mise à niveau.
