...
. Si des volumes sont hors ligne, comparez-les avec la liste des volumes hors ligne que vous avez répertoriée dans la section link:prepare-nodes-for-upgrade.html["Préparer les nœuds pour une mise à niveau"], et remettez en ligne les volumes hors ligne nécessaires à l'aide de la commande suivante, une fois pour chaque volume :
+
...
+
Vous pouvez comparer le résultat avec celui que vous avez enregistré dans la section link:prepare-nodes-for-upgrade.html["Préparer les nœuds pour une mise à niveau"].

. [[unset_maxwait]] Si des disques à chiffrement automatique sont utilisés dans la configuration et que vous avez défini la  `kmip.init.maxwait` variable à  `off` (par exemple, dans _Boot node2 with the replacement system modules_, link:boot-node2-with-a900-controller-and-nvs.html#boot_node2_step1["Étape 1"]), vous devez supprimer la variable :
+
...
+
Consultez link:other-references.html["Références"] pour accéder au _System Administration Reference_ pour des informations sur les SP et au _ONTAP 9 Command reference_ pour des informations détaillées sur la commande système  `service-processor network modify`.

. Si vous souhaitez configurer un cluster sans commutateur sur les nouveaux nœuds, reportez-vous à link:other-references.html["Références"] le _NetApp Support Site_ et suivez les instructions de _Transitioning to a two-node switchless cluster_.


.Une fois que vous avez terminé
Si le chiffrement du stockage est activé sur node1 et node2, complétez la section link:set-up-storage-encryption-new-module.html["Configurez Storage Encryption sur le nouveau module de contrôleur"]. Sinon, complétez la section link:decommission-old-system.html["Désaffectation de l'ancien système"].

.Et la suite ?
Après avoir confirmé que les nouveaux contrôleurs sont correctement configurés, link:set-up-storage-encryption-new-module.html["Configurez le chiffrement du stockage sur le nouveau module contrôleur"].
