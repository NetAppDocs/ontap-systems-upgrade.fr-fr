...
Les paramètres de port peuvent varier selon le modèle des nœuds. Vous devez configurer les ports et les LIF du nœud d'origine pour qu'ils soient compatibles avec l'utilisation et la configuration prévues du nouveau nœud. En effet, le nouveau nœud reproduit la même configuration au démarrage, ce qui signifie qu'au démarrage du nœud3, ONTAP tentera d'héberger les LIF sur les mêmes ports que ceux utilisés sur le nœud1.

Par conséquent, si les ports physiques du nœud1 ne correspondent pas directement aux ports physiques du nœud3, des modifications de la configuration logicielle sont nécessaires pour rétablir la connectivité du cluster, de la gestion et du réseau après le démarrage. De plus, si les ports du cluster du nœud1 ne correspondent pas directement aux ports du cluster du nœud3, le nœud3 pourrait ne pas rejoindre automatiquement le quorum lors de son redémarrage tant que vous n'aurez pas modifié la configuration logicielle pour héberger les LIF du cluster sur les ports physiques corrects.
...
+
Reportez-vous à link:other_references.html["Références"] pour obtenir un lien vers _Network Management_ ou le _ONTAP 9 Command reference_ pour plus d'informations.
