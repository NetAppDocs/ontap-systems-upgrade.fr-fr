...
Les paramètres de port peuvent varier selon le modèle des nœuds. Vous devez rendre la configuration des ports et des LIF du nœud d'origine compatible avec celle que vous prévoyez pour le nouveau nœud. Cela s'explique par le fait que le nouveau nœud rejoue la même configuration lors du démarrage, ce qui signifie que lorsque vous démarrez le nœud4, Data ONTAP tente d'héberger les LIF sur les mêmes ports que ceux utilisés sur le nœud2.

Par conséquent, si les ports physiques du nœud2 ne correspondent pas directement aux ports physiques du nœud4, des modifications de la configuration logicielle sont nécessaires pour rétablir la connectivité du cluster, de la gestion et du réseau après le démarrage. De plus, si les ports du cluster du nœud2 ne correspondent pas directement aux ports du cluster du nœud4, le nœud4 pourrait ne pas rejoindre automatiquement le quorum lors de son redémarrage tant qu'une modification de la configuration logicielle n'aura pas été effectuée pour héberger les LIF du cluster sur les ports physiques corrects.
...
+
Consultez link:other_references.html["Références"] pour accéder à _Network Management_ ou au _ONTAP 9 Command reference_ et consultez _Configuring failover settings on a LIF_ pour plus d'informations.
