...
CAUTION: À ce stade, la configuration MetroCluster n'est pas dans un état normal et vous pourriez avoir des erreurs à résoudre. Voir link:verify_health_of_metrocluster_config.html["Vérifier l'état de santé de la configuration MetroCluster"].
...
Si vous utilisez le NetApp Aggregate Encryption (NAE) ou le NetApp Volume Encryption (NVE) pour chiffrer les volumes sur le système que vous mettez à niveau, la configuration de chiffrement doit être synchronisée avec les nouveaux nœuds. Si vous ne restaurez pas key-manager, lorsque vous relocalisez les agrégats du nœud2 du nœud3 vers le nœud4 à l'aide d'ARL, les volumes chiffrés sont mis hors ligne.
