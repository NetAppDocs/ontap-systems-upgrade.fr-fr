...
* Si vous mettez à niveau un système avec des disques de stockage, vous devez compléter toute cette section, puis passer aux sections link:set_fc_or_uta_uta2_config_on_node3.html#configure-fc-ports-on-node3["Configurez les ports FC sur le nœud 3"] et link:set_fc_or_uta_uta2_config_on_node3.html#check-config-uta-uta2-node3["Vérifiez et configurez les ports UTA/UTA2 sur le nœud 3"], en saisissant les commandes à l'invite du cluster.
...
+
Saisissez la phrase secrète et les informations de sauvegarde que vous avez enregistrées précédemment dans la procédure. Reportez-vous à link:complete-preparation-for-upgrade.html#manage_storage_encryption_using_okm["Gérez le chiffrement du stockage à l'aide du gestionnaire de clés intégré"].
...
.Et la suite ?
* Si vous avez un système avec une configuration FC ou UTA/UTA2,link:set_fc_or_uta_uta2_config_on_node3.html["définir et configurer les ports FC ou UTA/UTA2 sur le nœud 3"] .
* Si vous n'avez pas de configuration FC ou UTA/UTA2,link:reassign-node1-disks-to-node3.html#reassign-node1-node3-app-step1["réaffecter les disques du nœud 1 au nœud 3, étape 1"] afin que le nœud 3 puisse reconnaître les disques du nœud 1.
* Si vous avez une configuration MetroCluster ,link:set_fc_or_uta_uta2_config_on_node3.html["définir et configurer les ports FC ou UTA/UTA2 sur le nœud 3"] pour détecter les disques attachés au nœud.
