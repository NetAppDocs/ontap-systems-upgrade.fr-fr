...
Si les ports physiques du nœud 1 ne correspondent pas directement aux ports physiques du nœud 3, la section suivante <<restore_network_configuration_on_node3,Restaurez la configuration du réseau sur le noeud 3>> doit être utilisée pour rétablir la connectivité réseau.
...
.Et la suite ?
Après avoir vérifié l'installation de node3, link:restore_key-manager_configuration_node3.html["restaurer la configuration du gestionnaire de clés sur node3"].
