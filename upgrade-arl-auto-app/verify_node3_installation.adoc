...
summary: 'Vérifiez que les ports physiques du nœud 1 sont correctement mappées sur le nœud 3 et restaurez la configuration réseau sur le nœud 3 lors de la mise à niveau des contrôleurs exécutant ONTAP 9.8 ou version ultérieure à l"aide de `system controller replace` commandes.' 
---
...
Si les ports physiques du nœud1 ne sont pas directement mis en correspondance avec les ports physiques du nœud3, la section suivante  network configuration on node3 doit être utilisé pour réparer la connectivité réseau.
