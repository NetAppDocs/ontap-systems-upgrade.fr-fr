...
Les LIF distantes gèrent le trafic vers les LUN SAN pendant la procédure de mise à niveau. Le déplacement des LIF SAN n'est pas nécessaire pour la santé du cluster ou du service pendant la mise à niveau. Les LIF SAN ne sont déplacées que si elles doivent être mappées sur de nouveaux ports. Vous vérifiez que les LIF sont saines et situées sur les ports appropriés après la mise en ligne de node3.
...
.Et la suite ?
Après avoir déplacé les agrégats non-racines et les LIF de données NAS appartenant au nœud1 du nœud2 vers le nœud3, link:relocate_non_root_aggr_nas_lifs_from_node2_to_node3.html["déplacer les agrégats non root et les LIF de données NAS du nœud2 vers le nœud3"].
