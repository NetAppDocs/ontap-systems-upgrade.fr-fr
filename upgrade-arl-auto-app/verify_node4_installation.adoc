...
Si les ports physiques du nœud 2 ne correspondent pas directement aux ports physiques du nœud 4, la section suivante <<restore_network_configuration_on_node4,Restaurez la configuration du réseau sur le noeud 4>> doit être utilisée pour rétablir la connectivité réseau.
...
.Et la suite ?
Après avoir vérifié l'installation de node4, link:restore_key-manager_config_node4.html["restaurer la configuration du gestionnaire de clés sur le nœud 4"].
