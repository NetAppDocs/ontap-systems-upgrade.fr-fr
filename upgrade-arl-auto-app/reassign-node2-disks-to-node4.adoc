...
.Étapes
. [[reassign-node2-node4-app-step1]]Vérifiez que le nœud 2 s'est arrêté au menu de démarrage et réaffectez les disques du nœud 2 au nœud 4 :
+
...
. Si le système entre dans une boucle de redémarrage avec le message `no disks found` , cela indique que le système a réinitialisé les ports FC ou UTA/UTA2 au mode cible et qu'il est donc incapable de voir les disques.  Sélectionnez l’une des tâches suivantes pour résoudre ce problème :
+
** Effectuer<<reassign-node2-node4-app-step3,Étape 3>> à<<reassign-node2-node4-app-step8,Étape 8>> sur node4
** Accéder à la sectionlink:verify_node4_installation.html["Vérifiez l'installation du node4"]
...
. [[reassign-node2-node4-app-step9]] Si vous effectuez une mise à niveau d'un système avec des disques externes vers un système qui prend en charge les disques internes et externes (systèmes AFF A800 , par exemple), définissez l'agrégat node2 comme agrégat racine pour garantir que node4 démarre à partir de l'agrégat racine de node2.  Pour définir l'agrégat racine, accédez au menu de démarrage sur node4 et sélectionnez l'option `5` pour entrer en mode maintenance.
+
...
.Et la suite ?
Après avoir réaffecté les disques du nœud 2 au nœud 4, link:verify_node4_installation.html["vérifier l'installation de node4"].
